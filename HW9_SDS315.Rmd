...
The 95% confidence interval for the difference in proportions in voting for those who did and didn't get a call is `r round(turnout_diffmean_ci$lower, 2)` - `r round(turnout_diffmean_ci$upper, 2)`

### Part B

#### Voting in 1996
```{r 2a_voted1996, echo=FALSE}
ggplot(filter(turnout_data, GOTV_call ==1)) + geom_bar(aes(x = factor(voted1996)))  + labs(x = "Voted in 1996", title ="Voting in 1996 when getting a GOTV Call")

ggplot(filter(turnout_data, GOTV_call ==0)) + geom_bar(aes(x = factor(voted1996)))  + labs(x = "Voted in 1996", title ="Voting in 1996 when not getting a GOTV Call")
```
...
ggplot(turnout_data) + geom_bar(aes(x = factor(GOTV_call))) + facet_wrap(~MAJORPTY) + labs(x = "Has Party membership", title ="Party membership effect on getting GOTV call")
```
...
#### Age
```{r 2A_AGE, echo= FALSE}
ggplot(turnout_data) + geom_boxplot(aes(group = GOTV_call, AGE)) + labs(title = "Average Age by if they Voted in 1998 and if they got a GOTV Call", y = "If Voted in 1998")
```
...
ggplot(GOTV_matches) + geom_bar(aes(x = factor(GOTV_call))) + facet_wrap(~voted1996) + labs(x = "Voted in 1996", title ="Voting in 1996 effect on GOTV call")

ggplot(GOTV_matches) + geom_bar(aes(x = factor(GOTV_call))) + facet_wrap(~MAJORPTY) + labs(x = "Has Party membership", title ="Party membership effect on GOTV call")

ggplot(GOTV_matches) + geom_boxplot(aes(group = GOTV_call, AGE)) + labs(title = "Average Age by GOTV call", y = "If Got GOTV call")
```
